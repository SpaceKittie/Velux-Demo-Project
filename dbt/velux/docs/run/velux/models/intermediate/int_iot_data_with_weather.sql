        ...
        create or replace transient table VELUX_DEV.STAGING.int_iot_data_with_weather
  ...
  (
    ...
    dew_point_temperature_celsius float,
    mean_sea_level_pressure_hpa float,
    wind_speed_ms float,
    visibility_km float,
    solar_irradiance_w_m2 float,
    uv_index float,
    sunshine_duration_minutes float,
    precipitation_type_description text
         ...
         as
        (
    select event_timestamp, country, city, device_id, room, sensor_type, sensor_value, outdoor_temperature_celsius, dew_point_temperature_celsius, mean_sea_level_pressure_hpa, wind_speed_ms, visibility_km, solar_irradiance_w_m2, uv_index, sunshine_duration_minutes, precipitation_type_description
    from (
